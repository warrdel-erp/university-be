...
export const getSingleStudentDetail = async (req, res) => {
  ...
  try {
    ...
    console.error(
      `Error in getting ${studentId} details , single student details:`,
      error,
    );
    res.status(500).send("Internal Server Error");
  }
...
export const getEmptyEnrollNumber = async (req, res) => {
  try {
    const result =
      await studentService.getEmptyEnrollNumber(getAcademicYearId());
    res.status(200).send(result);
  } catch (error) {
...
export const sectionStudentMapping = async (req, res) => {
  ...
  try {
    if (!(studentId && classSectionTermId)) {
      return res
        .status(400)
        .send("studentId and classSectionTermId are required");
    }
...
export const promotionPassed = async (req, res) => {
  const data = req.body;

  try {
    const passOne = async (payload) => {
      if (!payload.studentId) {
        const error = new Error("studentId is required for all students.");
        error.statusCode = 400;
        throw error;
      }
      return studentService.promotionPassed(payload);
    };

    if (Array.isArray(data)) {
      const results = [];
      for (const student of data) {
        const result = await passOne(student);
        results.push({
          success: true,
          message: "Student marked as passed and alumni",
          data: result,
        });
      }
      return res.status(200).json(results);
    ...
    const result = await passOne(data);
    return SuccessResponse(
      res,
      200,
      "Student marked as passed and alumni",
      result,
    );
  } catch (error) {
    console.error("Error in promotionPassed:", error);
    return ErrorResponse(
      res,
      error.statusCode || 500,
      error.message || "Internal Server Error",
...
export const getPromotionAvailableSectionRepeat = async (req, res) => {
  try {
    const { courseId, term, classSectionTermId } = req.query;

    const data = await studentService.getAvailableRepeatSections({
      courseId,
      term,
      classSectionTermId,
    });
    ...
    return SuccessResponse(
      res,
      200,
      "Repeat class sections fetched successfully",
      data,
    );
  } catch (error) {
    return ErrorResponse(
...
export const promotionFailed = async (req, res) => {
  const data = req.body;
  const createdBy = req.user.userId;

  try {
    const failOne = (payload) =>
      studentService.promotionFailed({ ...payload, createdBy });

    if (Array.isArray(data)) {
      const results = [];
      for (const student of data) {
        const result = await failOne(student);
        results.push({
          success: true,
          message:
            "Student marked as failed and retained in same term for next academic year",
          data: result,
        });
      }
      return res.status(200).json(results);
    ...
    const result = await failOne(data);
    return SuccessResponse(
      res,
      200,
      "Student marked as failed and retained in same term for next academic year",
    ...
    console.error("Error in promotionFailed:", error);
    return ErrorResponse(
      res,
      error.statusCode || 500,
      error.message || "Internal Server Error",
...
export const getPromotionStudentList = async (req, res) => {
  try {
    const { page, limit, programCourseId, studentSearch, promotionTerm } =
      req.query;
