...
export async function addAsset(req, res) {
  try {
    const row = await assetService.addAsset(req.body);
...
export async function getAllAsset(req, res) {
  try {
    const { data, pagination } = await assetService.listAssets(req.query);
...
export async function previewAssetCode(req, res) {
  try {
    const data = await assetService.previewAssetCode(req.query);
...
export async function getSingleAssetDetails(req, res) {
  try {
    ...
    const row = await assetService.getSingleAsset(assetId);
    if (!row) {
...
export async function updateAsset(req, res) {
  try {
    ...
    const row = await assetService.updateAsset(assetId, req.body);
...
export async function deleteAssetInventoryItem(req, res) {
  try {
    ...
    await assetService.deleteAssetInventoryItem(assetInventoryItemId);
