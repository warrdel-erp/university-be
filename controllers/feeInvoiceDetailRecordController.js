...
export async function addFeeInvoiceDetailRecord(req, res) {
  ...
  try {
    const feeInvoice = await feeInvoiceRecordService.addFeeInvoiceDetailRecord(
      feeInvoiceArray,
      createdBy,
      updatedBy
    );
...
export async function getAllFeeInvoiceDetailRecord(req, res) {
  ...
  try {
    const feeInvoice = await feeInvoiceRecordService.getAllFeeInvoiceDetailRecord({ academicYearId });
...
export async function getSingleFeeInvoiceDetailRecord(req, res) {
  try {
    ...
    const feeInvoice = await feeInvoiceRecordService.getSingleFeeInvoiceDetails(feeInvoiceId);
    if (feeInvoice) {
      res.status(200).json(feeInvoice);
    } else {
