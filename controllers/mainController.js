...
export const addCourse = async (req, res) => {
  try {
    ...
    const statusCode = /required|Unknown term|not found|scope/i.test(message)
      ? 400
      : 500;
  ...
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .send(
          "University Id,instituteId, course Id and academicYearId is required",
        );
    }
...
export const addClassSections = async (req, res) => {
  try {
    const result = await mainServices.addClassSections(
      req.body,
      req.user.userId,
    );
    ...
    const message = error.message || "Internal Server Error";
    const statusCode =
      /required|found|Active institute|already exists|must be between|No program terms|Duplicate section/.test(
        message,
      )
        ? 400
        : 500;
...
export const getClassSections = async (req, res) => {
  try {
    const rawClassSectionId =
      req.query.classSectionId ?? req.query.classSectionsId;
...
export const getClassSectionSpecific = async (req, res) => {
  try {
    ...
    const result = await mainServices.getClassSectionSpecific(
      campusId,
      undefined,
      undefined,
      courseId,
...
export const getSectionSubjectMapper = async (req, res) => {
  try {
    const { term, courseId, search, page, limit, academicYearId } = req.query;
    const result = await mainServices.getSectionSubjectMapper({
...
export const subjectExcel = async (req, res) => {
  try {
    ...
    if (!(courseId && getTenantStore().instituteId && academicYearId)) {
      return res
        .status(400)
        .send("academicYearId, courseId and instituteId is required");
    }
    ...
    const result = await mainServices.subjectExcel(
      excelData,
      courseId,
      academicYearId,
      specializationId,
...
export const getClassSectionRecordBatches = async (req, res) => {
  try {
    const { courseId } = req.query;
    const result = await mainServices.getClassSectionRecordBatches({
      courseId,
    });
    return SuccessResponse(
      res,
      200,
      "Class section batches fetched successfully",
      result,
    );
  } catch (error) {
    console.error("Error in getClassSectionRecordBatches:", error);
    const statusCode = error.statusCode || 500;
    return ErrorResponse(
      res,
      statusCode,
...
export const getClassSectionsByFilter = async (req, res) => {
  try {
    ...
    const result = await mainServices.getClassSectionsByFilter(
      sessionId,
      courseId,
    );
    return res.status(200).send(result);
