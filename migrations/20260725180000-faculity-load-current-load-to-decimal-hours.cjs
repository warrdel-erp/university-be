...
/**
 * faculity_load.current_load: STRING → DECIMAL(10,2) hours
 * Sanitize concat garbage, then seed from week-template cell teachers × period_length / 60.
...
module.exports = {
  async up(queryInterface, Sequelize) {
    await queryInterface.sequelize.transaction(async (transaction) => {
      ...
      // A. Sanitize corrupt STRING values before type change
      await queryInterface.sequelize.query(
        `
        UPDATE \`${TABLE}\`
        SET current_load = '0'
        WHERE current_load IS NULL
           OR TRIM(current_load) = ''
           OR current_load NOT REGEXP '^[0-9]+(\\.[0-9]+)?$'
           OR CAST(current_load AS DECIMAL(20, 2)) > 10000
        `,
        { transaction },
      );

      // B. Alter to DECIMAL hours
      ...
      // C. Seed hours from week-template assignments
      await queryInterface.sequelize.query(
        `
        UPDATE \`${TABLE}\` fl
