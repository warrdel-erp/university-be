...
/**
 * Creates org_position_head (user holding an org_position).
 * Depends on: 20260722190000-create-org-position
 * Idempotent: skips create if table already exists.
 * Also drops legacy org_position_assignment if present.
 */
...
module.exports = {
  async up(queryInterface, Sequelize) {
    if (!(await tableExists(queryInterface, 'org_position'))) {
      throw new Error('org_position must exist before creating org_position_head. Run create-org-position migration first.');
    }

    if (await tableExists(queryInterface, 'org_position_assignment')) {
      await queryInterface.dropTable('org_position_assignment');
    }
