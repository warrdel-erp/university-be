...
import {
  addtimeTableCreate,
  cloneTimeTableRoutine,
  gettimeTableCreateDetails,
  getSingletimeTableCreateDetails,
  addtimeTableMapping,
  getTimeTableMappingDetail,
  getSingletimeTableMappingDetail,
  getTimeTableCellData,
  updatetimeTableCreate,
  getTimeTableElective,
  publishTimeTable,
  updateSimpleTeacherMappingController,
  deletetimeTableMapping,
  deleteTimeTableTeacherController,
  ClassSubjectCount,
  changeTimeTableCreate,
...
const getTimeTableByCourseAndSectionQuerySchema = z
  .object({
    courseId: positiveIntegerId,
  ...
  .refine(
    (data) =>
      data.timeTableType === "elective" || data.classSectionTermId != null,
    { message: "classSectionTermId is required", path: ["classSectionTermId"] },
  );

const addTimeTableCreateSchema = z
  .object({
  ...
  .refine(
    (data) =>
      data.timeTableType === "elective" ||
      data.classSectionTermId != null ||
      data.academicGroupId != null,
    {
      message: "Either classSectionTermId or academicGroupId is required",
      path: ["classSectionTermId"],
    },
  )
  .refine(
    (data) =>
...
const updatePeriodsSchema = z
  .array(updatePeriodItemSchema)
...
const addTimeTableMappingSchema = mappingBodySchema.superRefine((body, ctx) => {
  ...
  if (isCopy && body.copyTarget == null) {
    ctx.addIssue({
      ...
      message:
        "copyTarget is required when sourceTimeTableCellId is sent (nextPeriod | nextDay)",
      path: ["copyTarget"],
    });
  }
  ...
  if (!isCopy && !hasSlots && !hasCell) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message:
        "Send sourceTimeTableCellId+copyTarget, slots[], or timeTableRoutineId with timeTableCreationId and period",
...
const updateSimpleTeacherMappingSchema = z
  ...
  .refine(
    (items) =>
      items.every((item) => item.isNew !== true || item.userId != null),
...
const updateDateWiseCellSchema = z
  .object({
  ...
  .refine(
    ...
    {
      message:
        "At least one of userId, subjectId, electiveSubjectId, classRoomSectionId is required",
    },
  )
  .refine(
    (body) =>
      (body.userId == null) === (body.timeTableCellTeachersDateWiseId == null),
    {
      message:
        "userId and timeTableCellTeachersDateWiseId must be sent together",
      path: ["timeTableCellTeachersDateWiseId"],
    },
  );
...
router.get(
  ...
  validate({ query: getTimeTableCreateListQuerySchema }),
  gettimeTableCreateDetails,
);
...
router.get(
  ...
  userAuth,
  checkAccess(PERMISSIONS.CREATE_TIME_TABLE_VIEW.value, null),
  validate({ query: getSingleQuerySchema }),
  getSingletimeTableCreateDetails,
);
router.get(
  "/create",
  userAuth,
  checkAccess(PERMISSIONS.CREATE_TIME_TABLE_VIEW.value, null),
  validate({ query: getTimeTableByCourseAndSectionQuerySchema }),
  getTimeTableByCourseAndSection,
);
...
router.get(
  ...
  userAuth,
  checkAccess(PERMISSIONS.CREATE_TIME_TABLE_VIEW.value, null),
  validate({ query: getRoutineSchema }),
  getRoutineByClassSectionId,
);

router.get(
  "/getRoutineByAcademicGroup",
  userAuth,
  checkAccess(PERMISSIONS.CREATE_TIME_TABLE_VIEW.value, null),
  validate({ query: getRoutineByAcademicGroupSchema }),
  getRoutineByAcademicGroupId,
);
...
router.get(
  ...
  userAuth,
  checkAccess(PERMISSIONS.CREATE_TIME_TABLE_VIEW.value, null),
  validate({ query: getDateWiseCellsQuerySchema }),
  getDateWiseCellsBySection,
);
...
router.post(
  ...
  validate({ body: addTimeTableCreateSchema }),
  addtimeTableCreate,
);
...
router.patch(
  ...
  userAuth,
  checkAccess(PERMISSIONS.CREATE_TIME_TABLE_EDIT_ROUTINE.value, null),
  validate({ body: patchTimeTableCreateSchema }),
  changeTimeTableCreate,
);
...
router.delete(
  ...
  validate({ query: deleteTimeTableRoutineQuerySchema }),
  deleteTimeTableRoutine,
);
...
router.post(
  ...
  userAuth,
  checkAccess(PERMISSIONS.CREATE_TIME_TABLE_CREATE_TIMETABLE.value, null),
  validate({ body: cloneRoutineSchema }),
  cloneTimeTableRoutine,
);
...
router.patch(
  ...
  userAuth,
  checkAccess(PERMISSIONS.CREATE_TIME_TABLE_EDIT_ROUTINE.value, null),
  validate({ query: publishTimeTableQuerySchema }),
  publishTimeTable,
);
...
router.post(
  ...
  userAuth,
  checkAccess(PERMISSIONS.CREATE_TIME_TABLE_CREATE_TIMETABLE.value, null),
  validate({ body: addTimeTableMappingSchema }),
  addtimeTableMapping,
);
...
router.get(
  ...
  userAuth,
  checkAccess(PERMISSIONS.CREATE_TIME_TABLE_VIEW.value, null),
  validate({ body: getTimeTableMappingBodySchema }),
  getTimeTableMappingDetail,
);
router.get(
  "/single/mapping",
...
router.patch(
  ...
  userAuth,
  checkAccess(PERMISSIONS.CREATE_TIME_TABLE_EDIT_ROUTINE.value, null),
  validate({ body: updateTimeTableMappingSchema }),
  updatetimeTableCreate,
);
router.patch(
  ...
  userAuth,
  checkAccess(PERMISSIONS.CREATE_TIME_TABLE_EDIT_ROUTINE.value, null),
  validate({ body: updateSimpleTeacherMappingSchema }),
  updateSimpleTeacherMappingController,
);
router.delete(
  "/mapping",
  userAuth,
  checkAccess(PERMISSIONS.CREATE_TIME_TABLE_DELETE_ROUTINE.value, null),
  validate({ query: deleteTimeTableMappingQuerySchema }),
  deletetimeTableMapping,
);
router.delete(
  "/mapping/teacher",
  userAuth,
  checkAccess(PERMISSIONS.CREATE_TIME_TABLE_DELETE_ROUTINE.value, null),
  validate({ query: deleteTimeTableTeacherQuerySchema }),
  deleteTimeTableTeacherController,
);
...
router.get(
  ...
  userAuth,
  checkAccess(PERMISSIONS.CREATE_TIME_TABLE_VIEW.value, null),
  validate({ query: getTimeTableCellDataQuerySchema }),
  getTimeTableCellData,
);
router.get(
  "/elective",
  userAuth,
  checkAccess(PERMISSIONS.CREATE_TIME_TABLE_VIEW.value, null),
  validate({ query: getTimeTableElectiveQuerySchema }),
  getTimeTableElective,
);
router.get(
  "/subjectCount",
  userAuth,
  checkAccess(PERMISSIONS.CREATE_TIME_TABLE_VIEW.value, null),
  validate({ query: classSubjectCountQuerySchema }),
  ClassSubjectCount,
);
