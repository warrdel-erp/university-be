...
router.get("/termsWithClassSections", userAuth, checkAccess(PERMISSIONS.COURSES.value), validate({ query: classSectionsGroupedSchema }), courseController.getTermsWithClassSections);

router.get("/:courseId/terms", userAuth, checkAccess(PERMISSIONS.COURSES.value), courseController.getTermOptionsByCourse);

router.delete("/:courseId", userAuth, checkAccess(PERMISSIONS.COURSES_DELETE.value), validate({ params: courseIdParamSchema }), courseController.deleteCourse);
