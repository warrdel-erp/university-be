import { decimalAdd, decimalSubtract, decimalMax } from "../utility/decimalMoney.js";
...
export function checkInvoices(st, reviewReasons) {
    ...
    const outstandingAmount = unpaidInvoicesList.reduce((sum, inv) => {
        const remaining = decimalSubtract(Number(inv.total) || 0, Number(inv.paidAmount) || 0);
...
export function checkAttendance(attendanceKPIs, reviewReasons) {
    const { totalClasses, minimumAttendanceRequired, attendancePercentage } = attendanceKPIs;
    if (totalClasses === 0) {
        reviewReasons.push({
...
export function calculateStudentEligibility(rawRecord) {
    ...
    const academicContext = {
        ...
        examinationSessionTermId:
            est?.examinationSessionTermId ?? null,
