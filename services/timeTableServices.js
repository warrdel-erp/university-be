...
export async function getTimetableListPrintData(filters = {}) {
    const rows = await timeTableRepository.getTimetableListPrintRows(filters);
    const result = [];
    ...
    for (const row of rows) {
        ...
        const isAcademicGroup = !!plain.academicGroupScopeId;
        ...
        if (plain.routines) {
            routineCount = plain.routines.length;
            for (const routine of plain.routines) {
        ...
        if (isAcademicGroup && academicGroupScope && academicGroupScope.groups) {
            totalGroup = academicGroupScope.groups.length;
            for (const group of academicGroupScope.groups) {
                totalStudent += group.students ? group.students.length : 0;
                facultyCount += group.users ? group.users.length : 0;
            }
        } else if (!isAcademicGroup && course) {
            const uniqueFacultyIds = new Set();
            if (plain.course && plain.course.courseSection) {
                totalGroup = plain.course.courseSection.length;
                for (const section of plain.course.courseSection) {
                    if (section.classSectionTerms) {
                        for (const term of section.classSectionTerms) {
                            if (term.studentTermPlacement) {
                                totalStudent += term.studentTermPlacement.length;
                            }
                        }
                    }
                    if (section.employeeSection) {
                        for (const emp of section.employeeSection) {
                            if (emp.userId) uniqueFacultyIds.add(emp.userId);
                        }
                    }
                }
            }
            facultyCount = uniqueFacultyIds.size;
        }
