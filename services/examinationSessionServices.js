...
export async function getExamSchedulesBySession(examinationSessionId, options = {}) {
  const slots = await examinationSessionRepository.findExaminationSessionSlots(examinationSessionId, options);
  if (!slots.length) return [];

  const schedules = await examinationSessionRepository.findExamSchedulesBySlotIds(
    slots.map((slot) => slot.examinationSessionSlotId),
    options,
  );

  // Compute student counts
  const sessions = uniqueValues(schedules.map(r => r.sessionId));
  const courses = uniqueValues(schedules.map(r => r.examSetupTypeTerm?.courseId));
  const terms = uniqueValues(schedules.map(r => r.examSetupTypeTerm?.term));
  const acedmicYears = uniqueValues(schedules.map(r => r.academicYearId));

  let counts = [];
  if (sessions.length > 0 && courses.length > 0 && terms.length > 0) {
      counts = await examScheduleRepository.getStudentCountsByGroups(sessions, courses, terms, acedmicYears);
  }

  const dateMap = new Map();

  for (const scheduleItem of schedules) {
    const plainSchedule = toPlain(scheduleItem);
    ...
    const { examinationSessionSlot, examSetupTypeTerm, ...cleanSchedule } = plainSchedule;
    
    // Compute Room Capacity
    cleanSchedule.roomCapacity = cleanSchedule.roomCapacities?.reduce((sum, rc) => sum + (rc.capacity || 0), 0) || 0;
    ...
    const countObj = counts.find(c =>
        c.sessionId === cleanSchedule.sessionId &&
        c.term === term &&
        c.courseId === courseId &&
  ...
  return [...dateMap.entries()].map(([examDate, slotMap]) => ({
    examDate,
