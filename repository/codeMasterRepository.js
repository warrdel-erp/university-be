...
function groupScopedTypesByCategory(categories, codeTypeRows) {
    ...
    return categories.map((cat) => {
        ...
        return {
            ...plain,
            codes: dedupeCodesByValue(codesByCategory.get(plain.employeeCodeMasterId) || []),
        };
    });
}
...
export async function addEmployeeCode(data) {
    try {
        ...
        if (!instituteId) {
            const error = new Error("Active institute is required to add code master value");
            error.statusCode = 400;
            throw error;
        }
        ...
        return scoped(model.employeeCodeMasterType).create({ ...data, instituteId });
    } catch (error) {
...
export async function getEmployeeCodesTypes(employeeCodeMasterId, key) {
    try {
        ...
        const codeTypes = await scoped(model.employeeCodeMasterType).findAll({
            ...
            order: [["employeeCodeMasterId", "ASC"], ["code", "ASC"]],
        });
