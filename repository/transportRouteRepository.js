...
export async function findAllTransportRoutes(page, limit, search) {
  try {
    ...
    if (search && String(search).trim()) {
      ...
      where[Op.or] = [
        { routeTitle: { [Op.like]: searchTerm } },
      ];
    }

    const { count, rows } = await scoped(model.transportRouteModel).findAndCountAll({
