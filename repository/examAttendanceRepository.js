...
if (!model.examScheduleRoomCapacityModel.associations.examInvigilatorAssignments) {
  model.examScheduleRoomCapacityModel.hasMany(model.examInvigilatorAssignmentModel, {
...
export async function updateExamAttendances(attendances) {
  ...
  for (const record of attendances) {
    ...
    if (updatedRows > 0) {
      const updatedRecord = await scoped(model.examAttendanceModel).findByPk(
        examAttendanceId,
      );
...
export async function findAndCountSchedules(
  where,
  subjectWhere,
  limit,
  offset,
) {
  const result = await scoped(model.examScheduleModel).findAndCountAll({
    where,
    attributes: [
      "examScheduleId",
      "examDate",
    ...
    include: [
      {
        ...
        required:
          Object.keys(subjectWhere).length > 0 || where[Op.or] ? true : false,
      },
      {
      ...
      },
      {
        ...
        include: [
          ...
          {
            model: model.examAttendanceModel,
            as: "examAttendances",
            attributes: ["examAttendanceId", "attendanceStatus"],
            required: false,
          },
          {
            model: model.studentExamSeatModel,
  ...
  result.rows.forEach((schedule) => {
    ...
    if (
      ...
      !slotIds.includes(schedule.examinationSessionSlotId)
    ) {
    ...
    (schedule.roomCapacities || []).forEach((room) => {
      if (
        ...
        !classRoomSectionIds.includes(room.classRoomSectionId)
      ) {
  ...
  if (
    examDates.length > 0 &&
    slotIds.length > 0 &&
    classRoomSectionIds.length > 0
  ) {
    assignments = await scoped(model.examInvigilatorAssignmentModel).findAll({
      where: {
  ...
  assignments.forEach((inv) => {
    assignmentSet.add(
      `${inv.examDate}_${inv.examinationSessionSlotId}_${inv.classRoomSectionId}`,
    );
  });

  result.rows.forEach((schedule) => {
    (schedule.roomCapacities || []).forEach((room) => {
      const key = `${schedule.examDate}_${schedule.examinationSessionSlotId}_${room.classRoomSectionId}`;
      room.setDataValue("isInvigilatorAssigned", assignmentSet.has(key));
    });
  });
...
export async function updateRoomCapacityStatus(
  examScheduleRoomCapacityId,
  status,
  transaction = null,
) {
...
export async function createAttendance(attendanceData, transaction) {
  return scoped(model.examAttendanceModel).create(attendanceData, {
    transaction,
  });
}
...
export async function getAttendanceStatusByExamSchedule(
  examScheduleId,
  roomCapacityIds,
) {
...
export async function getInvigilatorsByRooms({
  examDate,
  examinationSessionSlotId,
  classRoomSectionIds,
}) {
...
export async function getAttendancesWithStudent(
  examScheduleId,
  examScheduleRoomCapacityId,
) {
  return scoped(model.examAttendanceModel).findAll({
...
export async function getAttendanceCountsByRoom(examScheduleId) {
  return scoped(model.examAttendanceModel).findAll({
    where: { examScheduleId },
    attributes: [
      "examScheduleRoomCapacityId",
      "attendanceStatus",
      [sequelize.fn("COUNT", sequelize.col("exam_attendance_id")), "count"],
    ],
    group: ["examScheduleRoomCapacityId", "attendanceStatus"],
    raw: true,
  });
}

export async function getAttendanceCountsForRoomCapacities(roomCapacityIds) {
  if (!roomCapacityIds || roomCapacityIds.length === 0) return [];
  return scoped(model.examAttendanceModel).findAll({
    where: {
      examScheduleRoomCapacityId: { [Op.in]: roomCapacityIds }
...
export async function getInvigilatorAssignmentsForSchedules({ examDates, slotIds, classRoomSectionIds }) {
  if (!examDates.length || !slotIds.length || !classRoomSectionIds.length) return [];
  return scoped(model.examInvigilatorAssignmentModel).findAll({
    where: {
      examDate: { [Op.in]: examDates },
      examinationSessionSlotId: { [Op.in]: slotIds },
