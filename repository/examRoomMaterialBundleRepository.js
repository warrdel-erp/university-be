...
export async function getBundleList(filters, pagination) {
  ...
  const { count, rows } = await scoped(model.examScheduleRoomCapacityModel).findAndCountAll({
    subQuery: false,
        ...
        include: [
          {
            ...
            attributes: ["userId", "role", "examDate", "examinationSessionSlotId"],
            required: false,
            where: sequelize.and(
              sequelize.where(sequelize.col("classRoom.examInvigilatorAssignments.exam_date"), "=", sequelize.col("examSchedule.exam_date")),
              sequelize.where(sequelize.col("classRoom.examInvigilatorAssignments.examination_session_slot_id"), "=", sequelize.col("examSchedule.examination_session_slot_id"))
            ),
            include: [
              {
                model: model.users,
          ...
          },
          {
            ...
            required: false,
            where: sequelize.and(
              sequelize.where(sequelize.col("classRoom.materialBundles.exam_date"), "=", sequelize.col("examSchedule.exam_date")),
              sequelize.where(sequelize.col("classRoom.materialBundles.examination_session_slot_id"), "=", sequelize.col("examSchedule.examination_session_slot_id"))
            ),
            include: [
              {
...
export async function findBundleByMapping(examDate, examinationSessionSlotId, classRoomSectionId, transaction = null) {
  return scoped(model.examRoomMaterialBundleModel).findOne({
